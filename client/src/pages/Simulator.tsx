...
import { createAudioDataUrl, latestAgentMessage, selectPreferredVoice, speechPlaybackErrorMessage } from "@/lib/speech";
...
export default function Simulator() {
  ...
  const [speechStatus, setSpeechStatus] = useState<"idle" | "generating" | "speaking" | "fallback" | "error">("idle");
  const [activeVoiceName, setActiveVoiceName] = useState("Alice · ElevenLabs");
  ...
  const start = trpc.reception.start.useMutation({ onSuccess: data => { playbackSequenceRef.current += 1; audioRef.current?.pause(); lastSpokenMessageIdRef.current = null; setCallData(data); } });
  ...
  const { mutateAsync: synthesizeSpeech } = trpc.reception.speech.useMutation();
  const context = (callData?.call.context ?? {}) as Record<string, unknown>;
  const active = callData?.call.status === "active";
  const speechSupported = typeof window !== "undefined" && "speechSynthesis" in window;
  const voiceNeedsAttention = speechStatus === "error";
  const voiceLabel = !voiceEnabled ? "voice muted" : speechStatus === "generating" ? "preparing voice" : speechStatus === "speaking" ? "speaking" : speechStatus === "fallback" ? "device fallback" : voiceNeedsAttention ? "voice unavailable" : activeVoiceName;
  ...
  const speakWithBrowserFallback = useCallback((body: string) => {
    if (!("speechSynthesis" in window)) {
      setSpeechStatus("error");
      toast.error("The natural voice could not be played, and this browser has no device voice fallback.");
      return;
    }
    ...
    speech.pitch = 1;
    speech.volume = 1;
    speech.onstart = () => setSpeechStatus("fallback");
  ...
  const speakMessage = useCallback(async (message: { id: number | string; body: string }, callId: number) => {
    ...
    const useFallback = (reason: unknown) => {
      ...
      console.warn("[ElevenLabs playback fallback]", reason);
      const guidance = voiceProviderGuidance(reason instanceof Error ? reason.message : String(reason));
      toast.warning(guidance.title, { description: guidance.detail });
      speakWithBrowserFallback(message.body);
    };
    try {
      const result = await synthesizeSpeech({ callId, messageId: Number(message.id) });
      if (playbackSequence !== playbackSequenceRef.current || !voiceEnabledRef.current) return;
      const audio = new Audio(createAudioDataUrl(result.mimeType, result.audioBase64));
      audioRef.current?.pause();
      if ("speechSynthesis" in window) window.speechSynthesis.cancel();
      audioRef.current = audio;
      setActiveVoiceName(result.voiceName.split(" - ")[0]?.replace(" — ", " · ") || "ElevenLabs");
      audio.onplaying = () => setSpeechStatus("speaking");
      audio.onended = () => setSpeechStatus("idle");
      audio.onerror = () => useFallback("Audio decoding or playback failed");
      await audio.play();
    } catch (error) {
      useFallback(error);
    }
  }, [speakWithBrowserFallback, synthesizeSpeech]);

  useEffect(() => {
    if (!voiceEnabled || !callData) return;
    const latest = latestAgentMessage(callData.messages);
    if (!latest || latest.id === lastSpokenMessageIdRef.current) return;
    void speakMessage(latest, callData.call.id);
  }, [callData, speakMessage, voiceEnabled]);
  ...
  const speakLatest = () => {
    const latest = latestAgentMessage(callData?.messages ?? []);
    if (latest && callData) void speakMessage(latest, callData.call.id);
  };
  ...
  return (
    <div className="mx-auto max-w-[1500px] enter-soft">
      ...
      {!callData ? <Card className="overflow-hidden border-0 bg-[#173e35] text-white shadow-soft"><CardContent className="surface-grid grid min-h-[520px] place-items-center p-8 text-center"><div className="max-w-xl"><div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-white/10"><PhoneCall className="h-7 w-7 text-emerald-200" /></div><p className="mt-7 text-xs font-bold uppercase tracking-[0.2em] text-emerald-200">ElevenLabs voice studio</p><h2 className="mt-3 font-display text-3xl font-semibold tracking-tight sm:text-4xl">Meet Clara, your dental AI receptionist.</h2><p className="mx-auto mt-4 max-w-lg text-sm leading-6 text-white/65">Start a call, type or speak as a patient, and hear Clara respond with a natural female voice while she checks grounded knowledge and real availability.</p><Button onClick={() => start.mutate()} disabled={start.isPending} className="mt-8 h-12 rounded-full bg-[#f4efdf] px-7 text-[#173e35] hover:bg-white">{start.isPending ? <Waves className="mr-2 h-4 w-4 animate-pulse" /> : <PhoneCall className="mr-2 h-4 w-4" />}Start simulated call</Button></div></CardContent></Card> : (
